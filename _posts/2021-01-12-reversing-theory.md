...
Reversing을 할 때 레지스터에 있는 값이 피연산자가 되는 것이 아닌 레지스터에 저장된 메모리 주소를 참조한 값이 피연산자가 되는 경우가 있다고 합니다. 이에 대한 개념은 C 언어에서 pointer를 생각하면 쉽게 이해할 수 있으나 그 이후 추가 설명이 이해하기 어려웠었습니다.

먼저 C 언어에서 pointer를 사용하기 위해 * 문자를 사용하듯 Assembly에서는 []로 레지스터를 감싸서 표현하였습니다. 예를 들어 rcx 레지스터에 저장된 주소의 경우 다음과 같이 표기하였습니다.

>`[rcx]`

만약 레지스터에 들어있는 값을 기준으로 n 만큼 떨어진 주소를 사용하는 경우 산술 연산자를 사용하여 이를 표기할 수도 있습니다.
...
주로 인덱스로 참조하는 형식으로 아래와 같은 for문이 있을 경우 reg1이 arr을, reg2가 i를 의미한다고 합니다.

```c
for(i=0;i<10;i++) arr[i]
```

주로 구조체와 같은 경우 다음과 같은 형식을 확인할 수 있다고 합니다.

>`[reg1+reg2*i+j]`

위 형식을 이해하기 쉽게 표현하면 다음과 같이 표현할 수 있었습니다.

>`[reg1+(reg2*i)+j]`

즉, 위에서 설명되었던 형태가 혼합된 것임을 알 수 있습니다.
